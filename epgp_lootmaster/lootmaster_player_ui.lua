...
function LootMaster:InitUI()
    ...
    titletext:SetText( string.format("EPGPLootMaster %s переклад Nebo. Переклад аддону власність серверу FreedomWoW UA", self:GetVersionString() ) )    
...
function LootMaster:CreateLootSelectFrame()
    ...
    b:SetScript("OnUpdate", function( o, elapsed )
        ...
        frame.lblTimeout:SetText( format("%s секунд до пропуску", ceil(frame.data.timeoutLeft)) )
    ...
    btnNote:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Добавити замітку")
        GameTooltip:AddLine("Клацніть щоб добавити замітку, щоб надіслати Лідеру.",.8,.8,.8,1, true)
        GameTooltip:Show()
    end)
...
function LootMaster:UpdateLootUI()
    ...
    for i, data in ipairs(self.lootList) do repeat
        ...
        lootFrame.lblInfo:SetText(format("Рівень: %s, GP: %s", data.ilevel or -1, gp))
        lootFrame.lblLooter:SetText(format("Лутає: %s", data.lootmaster or 'unknown'))
