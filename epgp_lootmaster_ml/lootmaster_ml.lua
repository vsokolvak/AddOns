...
StaticPopupDialogs["EPGPLOOTMASTER_ASK_TRACKING"] = {
	text = '- - - - EPGPLootmaster FreedomWoW UA - - - -\r\n\r\nВи РЛ і хочете розподіляти лут по системі EP/GP?\r\n\r\n(Для виклику меню програми, наберіть /lm config щоб змінити налаштування)',
	button1 = YES,
	button2 = NO,
	OnAccept = function()
		LootMasterML:EnableTracking()
        LootMaster:Print('Ви приєднались до рейдової групи гільдії з системою луту EP/GP')
	end,
	OnCancel = function()
...
function LootMasterML:AskCandidateIfNeeded( link, candidate )
	...
	if candidate == 'RAID' or candidate == 'PARTY' then
        ...
        SendChatMessage( format('%sВи не можете претендувати на здобич. Встановіть аддон з діскорд серверу https://discord.gg/AVgnYPSkb9', MsgPrefix or '', loot.link or ''), candidate );
    ...
    elseif candidate ~= UnitName('player') then
        ...
        SendChatMessage( format('%sВи не можете претендувати на здобич. Встановіть аддон з діскорд серверу https://discord.gg/AVgnYPSkb9', MsgPrefix or '', loot.link or ''), 'WHISPER', nil, candidate );
...
function LootMasterML:SetManualResponse(loot, candidate, response)
    ...
    self:SendWhisperResponse(format('Ваш вибір для %s був переглянутий РЛ і встановлений як %s.', loot.link or 'unknown item', sResponse or 'unknown'), candidate);
...
function LootMasterML:OPEN_MASTER_LOOT_LIST()
    ...
    if LootMaster.db.profile.AutoLootThreshold~=0 and LootMaster.db.profile.AutoLooter~='' and self.lootTable[lootID].autoLootable then
        ...
        if IsAltKeyDown() then
            self:Print('Немає автоматичного розподілення (alt+click активувати)')
        else
            isAutoLooted = true
            if self:IsCandidate( lootID, LootMaster.db.profile.AutoLooter or '' ) then
                self:Print(format('Автоматичне розподілення %s і %s', link or 'nil', LootMaster.db.profile.AutoLooter or 'nil'))
                ...
                self:Print(format('Автоматичне розподілення %s і %s не відбулось. Не знайдено кандидата на лут.', link or 'nil', LootMaster.db.profile.AutoLooter or 'nil'))
            end
        end
    end
