...
local sstScrollCols = {
       ...
       { ["name"] = "Голосування",		    ["width"] = 210, ["align"] = "LEFT", 	  ["defaultsort"] = "desc", ["sort"] = "desc", ["color"] = {["r"] = 0.25, ["g"] = 1.00, ["b"] = 0.25, ["a"] = 1.0 }, ["sortnext"]=10 }, --,
       ...
       { ["name"] = "Рол",		        ["width"] = 35,  ["align"] = "RIGHT",   ["defaultsort"] = "asc", ["sort"] = "asc", ["color"] = epgpColor},
       ...
       { ["name"] = "Рівень",		        ["width"] = 60,  ["align"] = "CENTER",  ["bgcolor"] = gearBgColor },
...
function LootMasterML:GetFrame()
    ...
    titleFrame:SetScript("OnEnter", function() self:ShowInfoPopup("EPGPLootmaster сервера FreedomWoW", "Клікніть та утримуйте курсор для переміщення вікна.", "Клацніть двіччі для згортання вікна.") end)
	...
	titletext:SetText( string.format("EPGPLootMaster %s переклад Nebo. Переклад аддона власність серверa FreedomWoW UA", self:GetVersionString() ) )
	...
	titletext:SetText( string.format("Акт.предмети на кандидаті:", self:GetVersionString() ) )
    --#endregion
	...
	lblGPOverride:SetText( "Цінність в GP:" );
    ...
    tbGPValue:SetScript("OnEnter", function() self:ShowInfoPopup("Вартість GP", "Зміна вказаного значення GP для даної речі") end)
    ...
    btnAnnounce:SetScript("OnEnter", function() self:ShowInfoPopup( "Оголосити торги",
                                                                    "Клацніть по цьому значку для початку розподілення луту.",
                                                                    "Це автоматично відкриє вікно розподілення луту на інших клієнтах.") end)
    ...
    btnDiscard:SetScript("OnEnter", function() self:ShowInfoPopup( "Закрити вікно",
                                                                   "Клацніть, щоб закрити вікно і всі вибори кандидатів з Вашого списку.",
                                                                   "Використовуйте якщо не хочите приймати участь в розподіленні здобичі.") end)
	...
	btnDiscard:SetText("Закрити вікно")
    ...
    if UIDROPDOWNMENU_MENU_LEVEL == 1 then
        ...
        if loot.mayDistribute then
            ...
            if not loot.manual then
                ...
                info.text = format( 'Віддати лут і нарахувати %s GP', loot.gpvalue_manual or 0 );
                ...
                if loot.gpvalue2 and loot.gpvalue2~=0 then
                    info.isTitle = false;
                    info.disabled = not CanEditOfficerNote();
                    info.text = format( 'Віддати лут і нарахувати %s GP (100%%)', loot.gpvalue2 or 0 );
                    info.tooltipTitle = info.text;
                ...
                for i=1, loot.numButtons do
                    ...
                    if btn.gpValue then
                      ...
                      info.isTitle = false;
                      info.disabled = not CanEditOfficerNote();
                      info.text = format( 'Віддати лут і нарахувати %s GP %s (%s)', gp, btn.text, vs );
                      info.tooltipTitle = info.text;
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = 'Віддати лут безкоштовно';
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = 'Віддати лут на розпил';
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = '- Неможливо розподілити -';
                info.tooltipTitle = info.text;
            ...
            info.isTitle = false;
            info.disabled = false;
            info.text = 'Переролити персонажу';
            ...
            info.disabled = false;
            info.hasArrow = 1;
            info.text = 'Забір і видача луту вручну';
...
function LootMasterML:ShowGearInspectPopup( candidate, item )
    ...
    if not foundGear then
        self:ShowInfoPopup( candidate, 'Клацніть, щоб побачити обмундирування персонажа.' );
    end
end
...
function LootMasterML:ShowRollCellPopup( candidate, item )
    ...
    self:ShowInfoPopup( 'Випадковий рол',
                        'Ця опція потрібна лише тоді, коли у обох кандидатів однакове значення PR.',
                        'Це - простий звичайний рол, щоб вирішити, хто отримає здобич',
...
function LootMasterML:ShowRaidInfoLookup()
    ...
    if not frame then
        ...
        local titletext = titleFrame:CreateFontString(nil,"OVERLAY","GameFontNormalSmall")
        titletext:SetPoint("CENTER",titleFrame,"CENTER",0,1)
        titletext:SetText( string.format("EPGPLootMaster %s by Nebo <FreedomWoW> - Ukraine", LootMaster:GetVersionString() ) )
        frame.titleFrame = titleFrame
        --#endregion
